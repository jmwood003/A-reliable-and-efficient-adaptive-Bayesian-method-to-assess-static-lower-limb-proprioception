function [AllStarts, AllTrgts, AllAnswers] = ViconTMConnect_AFC(Ntrials, targets, TLstr)

rng('shuffle')
strtpos_sigma = 40;

%Set all the tagets by resampling without replacement Ntrials times
AllTrgts = [];
for t = 1:Ntrials
    AllTrgts = [AllTrgts, randsample(targets,length(targets))];
end

% %Fully randomized start positions
% AllStarts = round(normrnd(0,strtpos_sigma,[1,length(AllTrgts)]));

%Psudorandomized start positions
AllStarts = nan(1,length(AllTrgts));
for s = 1:length(targets)
    ctrgt = targets(s);
    idx = find(AllTrgts==ctrgt);
    tempstarts = round(normrnd(0,strtpos_sigma,[1,length(idx)]));
    while sum(tempstarts > ctrgt) ~= sum(tempstarts < ctrgt)
        tempstarts = round(normrnd(0,strtpos_sigma,[1,length(idx)]));
    end
    AllStarts(idx) = tempstarts;
end
...
%   fprintf( 'Frame Number: %d\n', Output_GetFrameNumber.FrameNumber );
  ...
  DeviceIndex = typecast( DeviceCount, 'int32' ) - 2;
...
%   Output_GetDeviceName.DeviceName
  ...
  DeviceOutputIndex = typecast( DeviceOutputCount, 'int32' ) - 7;
...
%   Output_GetDeviceOutputComponentName.DeviceOutputComponentName
  ...
  Output_GetDeviceOutputValue = MyClient.GetDeviceOutputValue( Output_GetDeviceName.DeviceName, Output_GetDeviceOutputComponentName.DeviceOutputName, Output_GetDeviceOutputComponentName.DeviceOutputComponentName);
  FZ_L = Output_GetDeviceOutputValue.Value();
  
  %Index Right Force plate
  DeviceCountR = MyClient.GetDeviceCount().DeviceCount;
  DeviceIndexR = typecast( DeviceCountR, 'int32' ) - 3;
...
%   Output_GetDeviceOutputComponentNameR.DeviceOutputComponentName
  ...
  %Move leg to start position (slowly)  
  if MkrDiff == startpos
      ...
      startpos = nan;
      %Get an end position
      endpos = AllTrgts(trial);
      
      pause(rand(1));
      
      %Move treadmill to new end position    
      if endpos < MkrDiff
          TMtestSpeed = 10;
      else
          TMtestSpeed = -10;
  ...
  elseif MkrDiff == endpos
       
      disp(['End Pos: ' num2str(endpos)]);
      ...
      answer = input(['Trial # ' num2str(trial) '; Response (r or l)?'],'s');
      while strcmp(answer,'r')==0 && strcmp(answer,'l')==0
          disp('incorrect response entered');
          answer = input(['Trial # ' num2str(trial) '; Response (r or l)?'],'s');
      end
      AllAnswers{trial} = answer;
      ...
      %Move treadmill
      if startpos < MkrDiff || endpos < MkrDiff
          TMtestSpeed = 10;
      else
          TMtestSpeed = -10;
      end
